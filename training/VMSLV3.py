...
class Config:
    ...
    num_epochs = 500
    patience = 70
    save_dir = "./yolo_training_resultsv96"
    log_dir = "./yolo_logsv96"   
    ...
    def get_model_save_path(cls):
        return os.path.join(cls.save_dir, "best_convnext_model.pth")  # 修改模型保存名称
...
def setup_logging():
    ...
    log_file = os.path.join(Config.log_dir, f"convnext_training_{timestamp}.log")  # 修改日志名称
    logging.basicConfig(
        level=logging.INFO,
    ...
    logger.info(f"ConvNeXtV2 Training log: {log_file}")
...
# 移除YOLO ROI提取器，替换为直接返回原图的处理器
class ImageProcessor:
    """简单的图像处理器，不进行ROI提取，直接返回原图"""
    def __init__(self, device):
        self.device = device
        get_logger().info("Using original images without ROI extraction")

    def process_clinic_image(self, image):
        """直接返回原图"""
        return image

    def process_wood_image(self, image):
        """直接返回原图"""
        return image

class VitiligoDataset(Dataset):  # 修改类名
    def __init__(self, root_dir, csv_file, processor, transform=None, image_size=(224, 224)):
        self.data = pd.read_csv(csv_file)
        self.root_dir = root_dir
        self.processor = processor  # 使用图像处理器替代ROI提取器
        ...
        get_logger().info(f"Dataset initialized with {len(self.group_keys)} sample pairs")
    ...
    def __getitem__(self, idx):
        ...
        try:
            # 直接使用原图，不进行ROI提取
            clinic_processed = self.processor.process_clinic_image(clinic_image)
            wood_processed = self.processor.process_wood_image(wood_image)
        except Exception as e:
            get_logger().warning(f"Image processing failed for {pair_id}: {e}")
            clinic_processed = clinic_image
            wood_processed = wood_image
        
        # 调整图像大小到指定尺寸
        clinic_processed = cv2.resize(clinic_processed, self.image_size)
        wood_processed = cv2.resize(wood_processed, self.image_size)
        
        if self.transform:
            augmented = self.transform(image=clinic_processed, image1=wood_processed)
            clinic_tensor = augmented['image']
            wood_tensor = augmented['image1']
        else:
            clinic_tensor = torch.tensor(clinic_processed.transpose(2, 0, 1), dtype=torch.float32) / 255.0
            wood_tensor = torch.tensor(wood_processed.transpose(2, 0, 1), dtype=torch.float32) / 255.0
...
class ConvNeXtV2Model(nn.Module):  # 修改类名
...
def train_model(model, train_loader, val_loader, config):
    ...
    patience_counter = 0
    logger.info("="*80)
    logger.info("CONVNEXTV2 TRAINING STARTED")  # 修改日志信息
    logger.info("="*80)
...
def main():
    ...
    logger.info("="*100)
    logger.info("CONVNEXTV2 VITILIGO CLASSIFICATION MODEL TRAINING")  # 修改日志信息
    logger.info("="*100)
    ...
    base_dataset = VitiligoDataset(  # 修改类名
        root_dir=Config.root_dir,
        csv_file=Config.csv_file,
        processor=processor,  # 使用图像处理器
    ...
    logger.info("Creating ConvNeXtV2 model...")  # 修改日志信息
    model = ConvNeXtV2Model(Config).to(Config.device)  # 修改类名
    ...
    logger.info("CONVNEXTV2 TRAINING COMPLETED SUCCESSFULLY!")  # 修改日志信息
    logger.info("="*100)

...
class Config:
    ...
    num_epochs = 500
    patience = 70
    save_dir = "./yolo_training_resultsv95"
    log_dir = "./yolo_logsv95"   
...
class YOLOConvNeXtV2Model(nn.Module):
    def __init__(self, config):
        ...
        # 移除交叉注意力机制，替换为简单的特征融合
        self.fusion_layer = nn.Sequential(
            nn.Conv2d(config.hidden_dim * 2, config.hidden_dim, kernel_size=1),
            nn.GELU()
        )
    ...
    def forward(self, c_img, w_img):
        ...
        # 简单拼接融合
        fused_features = self.fusion_layer(torch.cat([c_encoded, w_encoded], dim=1))
    ...
    def compute_losses(self, outputs, stability_labels, loss_weights):
        # 替换Focal Loss为交叉熵损失，不保留标签平滑
        criterion = nn.CrossEntropyLoss()  # 移除label_smoothing参数，使用默认配置
        cls_loss = criterion(outputs['cls_pred_f'], stability_labels)

...
class Config:
    ...
    det_json_dir = "../detection_json"
    seg_json_dir = "../segmentation_json"
    lr = 1e-4
    weight_decay = 1e-5
    ...
    stage_epochs = [80, 30, 30, 30, 100, 120]  
  
    stage_patience = [40, 20, 20, 20, 40, 60] 
    stage_lrs = [5e-5, 4e-5, 4e-5, 4e-5, 6e-5, 4e-5]  
    focal_alpha = 2      
    focal_gamma = 3      
...
class MultiTaskModel(nn.Module):
    def __init__(self, config):
        ...
        self.z_c_classifier = nn.Sequential(
            nn.Linear(config.hidden_dim, config.hidden_dim // 2),
            nn.GELU(),
            nn.Dropout(0.3),
            nn.Linear(config.hidden_dim // 2, config.num_classes)
        )
        self.z_w_classifier = nn.Sequential(
            nn.Linear(config.hidden_dim, config.hidden_dim // 2),
            nn.GELU(),
            nn.Dropout(0.3),
            nn.Linear(config.hidden_dim // 2, config.num_classes)
        )
        self.z_f_classifier = nn.Sequential(
            nn.Linear(config.hidden_dim, config.hidden_dim // 2),
            nn.GELU(),
            nn.Dropout(0.3),
            nn.Linear(config.hidden_dim // 2, config.num_classes)
...
class ContrastiveLoss(nn.Module):
    def __init__(self, device):
        ...
        self.pos_margin = nn.Parameter(torch.tensor(0.9, device=device))
        self.neg_margin = nn.Parameter(torch.tensor(0.2, device=device)) 
...
class StagedMultiTaskLoss(nn.Module):
    def __init__(self, config):
        ...
        self.log_vars = nn.ParameterDict({
            'log_var_det': nn.Parameter(torch.tensor(0.5).to(config.device)),      
            'log_var_seg': nn.Parameter(torch.tensor(0.5).to(config.device)),     
            'log_var_recon': nn.Parameter(torch.tensor(0.8).to(config.device)),   
            'log_var_class': nn.Parameter(torch.tensor(-0.3).to(config.device)),  
            'log_var_con': nn.Parameter(torch.tensor(-0.1).to(config.device))   
        })
    ...
    def _get_aux_classification_loss(self, outputs, class_labels):
        """计算辅助分类损失"""
        aux_class_loss = 0.0
        try:
            z_c_pred = self.z_c_classifier(outputs['z_c'])
            z_w_pred = self.z_w_classifier(outputs['z_w']) 
            z_f_pred = self.z_f_classifier(outputs['z_f'])
            
            aux_class_loss += self.class_loss_focal(z_c_pred, class_labels) * 0.3
            aux_class_loss += self.class_loss_focal(z_w_pred, class_labels) * 0.3
            aux_class_loss += self.class_loss_focal(z_f_pred, class_labels) * 0.4
        except Exception:
            aux_class_loss = torch.tensor(0.0, device=class_labels.device)
        return aux_class_loss

    def _get_detection_loss(self, outputs, det_targets):
        """计算检测损失"""
        det_loss = []
        ...
        if len(det_loss) > 0:
            return torch.stack(det_loss).mean() * (
                self.det_empty_weight if all(len(dt) == 0 for dt in det_targets) else 1.0)
        else:
            return torch.tensor(0.0, device=outputs['detection'].device)

    def _get_reconstruction_loss(self, outputs, clinic_img, wood_img):
        """计算重构损失"""
        recon_loss = 0.0
        ...
        return recon_loss / (2 * clinic_img.shape[0])
    ...
    def forward(self, outputs, targets, stage):
        ...
        if stage == 0:  
            ...
            losses['detection'] = self._get_detection_loss(outputs, det_targets)
            losses['segmentation'] = self.seg_loss(outputs['segmentation'], seg_targets)
            losses['reconstruction'] = self._get_reconstruction_loss(outputs, clinic_img, wood_img)
            losses['contrastive'] = self.contrastive_loss(outputs['z_c'], outputs['z_w'], class_labels)
            losses['aux_classification'] = self._get_aux_classification_loss(outputs, class_labels)

            task_losses = {
                'detection': losses['detection'],
                'segmentation': losses['segmentation'],
                'reconstruction': losses['reconstruction'],
                'classification': losses['classification'] + losses['aux_classification'],
                'contrastive': losses['contrastive']
            }

            task_to_logvar = {
                'detection': 'log_var_det',
                'segmentation': 'log_var_seg', 
                'reconstruction': 'log_var_recon',
                'classification': 'log_var_class',
                'contrastive': 'log_var_con'
            }
            
            for task_name, task_loss in task_losses.items():
                log_var_key = task_to_logvar.get(task_name)
                importance_weight = self.task_importance.get(task_name, 1.0)
                
                if log_var_key and log_var_key in self.log_vars:
                    precision = torch.exp(-self.log_vars[log_var_key])
                    weighted_loss = importance_weight * precision * task_loss + 0.5 * self.log_vars[log_var_key]
                    total_loss += weighted_loss
                else:
                    total_loss += importance_weight * task_loss
            ...
            precision_recon = torch.exp(-self.log_vars['log_var_recon'])
            precision_class = torch.exp(-self.log_vars['log_var_class'])
            
            total_loss += precision_recon * losses['reconstruction'] + 0.5 * self.log_vars['log_var_recon']
            total_loss += precision_class * (losses['classification'] + losses['aux_classification']) + 0.5 * self.log_vars['log_var_class']
            ...
            precision_det = torch.exp(-self.log_vars['log_var_det'])
            precision_class = torch.exp(-self.log_vars['log_var_class'])
            
            total_loss += precision_det * losses['detection'] + 0.5 * self.log_vars['log_var_det']
            total_loss += precision_class * (losses['classification'] + losses['aux_classification']) + 0.5 * self.log_vars['log_var_class']
            ...
            precision_seg = torch.exp(-self.log_vars['log_var_seg'])
            precision_class = torch.exp(-self.log_vars['log_var_class'])
            
            total_loss += precision_seg * losses['segmentation'] + 0.5 * self.log_vars['log_var_seg']
            total_loss += precision_class * (losses['classification'] + losses['aux_classification']) + 0.5 * self.log_vars['log_var_class']
...
def train_epoch(model, dataloader, criterion, optimizer, device, logger, stage, scaler=None):
    ...
    for clinic_img, wood_img, det_labels, det_targets, seg_targets, class_labels in pbar:
        ...
        if grad_norm > Config.max_grad_norm * 4 and Config.show_gradient_info:  # 只在非常大时才显示
            logger.info(f"Stage {stage} - Large gradient norm clipped: {grad_norm:.4f}")
        
        
       
        train_epoch.last_losses = losses
        train_epoch.last_gradients = {}
        for key, param in criterion.log_vars.items():
            if param.grad is not None:
                train_epoch.last_gradients[key] = param.grad.item()
        ...
        try:
            with torch.no_grad():
                ...
                main_probs = F.softmax(classification_logits, dim=1)
                
        
                try:
                    z_c_logits = model.z_c_classifier(outputs['z_c'])
                    z_w_logits = model.z_w_classifier(outputs['z_w'])
                    z_f_logits = model.z_f_classifier(outputs['z_f'])
                    
                    aux_c_probs = F.softmax(z_c_logits, dim=1)
                    aux_w_probs = F.softmax(z_w_logits, dim=1)
                    aux_f_probs = F.softmax(z_f_logits, dim=1)
                    
                  
                    ensemble_probs = (0.6 * main_probs + 
                                    0.1 * aux_c_probs + 
                                    0.1 * aux_w_probs + 
                                    0.2 * aux_f_probs)
...
def freeze_model_parts(model, stage):
    ...
    for name, child in model.named_children():
        if name == 'backbone':
            for stage in [child.stages_0, child.stages_1, child.stages_2]:
    ...
    if stage == 0:  
        ...
        for name, child in model.named_children():
            if name not in ['backbone', 'cross_attention', 'fusion_conv', 'reconstruction_head', 'classifier', 'z_c_classifier', 'z_w_classifier', 'z_f_classifier']:
                for param in child.parameters():
                    param.requires_grad = False
    elif stage == 2: 
        for name, child in model.named_children():
            if name not in ['backbone', 'cross_attention', 'fusion_conv', 'detection_head', 'classifier', 'z_c_classifier', 'z_w_classifier', 'z_f_classifier']:
                for param in child.parameters():
                    param.requires_grad = False
    elif stage == 3: 
        for name, child in model.named_children():
            if name not in ['backbone', 'cross_attention', 'fusion_conv', 'segmentation_head', 'classifier', 'z_c_classifier', 'z_w_classifier', 'z_f_classifier']:
                for param in child.parameters():
                    param.requires_grad = False
    elif stage == 4:
        for name, child in model.named_children():
            if name not in ['backbone', 'cross_attention', 'fusion_conv', 'classifier', 'z_c_classifier', 'z_w_classifier', 'z_f_classifier']:
                for param in child.parameters():
                    param.requires_grad = False
    ...
    for stage in range(num_stages):
        ...
        if os.path.exists(current_best_model_path):
            try:
                ...
                logger.info(f"Stage {stage}: Reusing existing criterion to maintain log_var parameter states")
            except Exception as e:
        ...
        if stage in [0, 4, 5]: 
            param_groups = [
                ...
                {'params': loss_params, 'lr': Config.stage_lrs[stage] * 0.2}, 
                {'params': contrastive_params, 'lr': Config.stage_lrs[stage] * 0.1}  
            ]
        else:  
            param_groups = [
                {'params': model_params, 'lr': Config.stage_lrs[stage] * 0.8},
        ...
        for epoch in range(Config.stage_epochs[stage]):
            ...
            if stage in [0, 4, 5]:
                logger.info(f"  [CLASSIFICATION FOCUS] Stage {stage} - Main task performance tracking")

            if stage in [0, 4, 5] and hasattr(train_epoch, 'last_losses'):
                try:
                    losses = train_epoch.last_losses
                    ...
                    for key, log_var in criterion.log_vars.items():
                        ...
                        grad_info = ""
                        if hasattr(train_epoch, 'last_gradients') and key in train_epoch.last_gradients:
                            grad_info = f" (grad: {train_epoch.last_gradients[key]:.6f})"
                        else:
                            grad_info = " (no grad)"
